...
const instanceColumns = `id, slug, name, description, connector_id, base_url, variables, enabled,
	allow_private_network, timeout_ms, rate_limit_per_min, max_concurrent, max_response_bytes,
...
func (r *instanceRepo) Create(ctx context.Context, i *domain.Instance) error {
	...
	_, err = r.write.ExecContext(ctx,
		...
		boolToInt(i.AllowPrivateNetwork), i.TimeoutMS, i.RateLimitPerMin, i.MaxConcurrent, i.MaxResponseBytes,
...
func (r *instanceRepo) Update(ctx context.Context, i *domain.Instance) error {
	...
	res, err := r.write.ExecContext(ctx,
		`UPDATE instances SET slug = ?, name = ?, description = ?, base_url = ?, variables = ?,
		   enabled = ?, allow_private_network = ?, timeout_ms = ?, rate_limit_per_min = ?,
		   max_concurrent = ?, max_response_bytes = ?, updated_at = ?, version = version + 1
		 WHERE id = ?`,
		i.Slug, i.Name, i.Description, i.BaseURL, vars, boolToInt(i.Enabled),
		boolToInt(i.AllowPrivateNetwork), i.TimeoutMS, i.RateLimitPerMin, i.MaxConcurrent,
		i.MaxResponseBytes, formatTime(i.UpdatedAt), i.ID)
	if err != nil {
...
func scanInstance(s scanner) (*domain.Instance, error) {
	...
	if err := s.Scan(&i.ID, &i.Slug, &i.Name, &i.Description, &i.ConnectorID, &i.BaseURL, &vars,
		&enabled, &allowPrivate, &i.TimeoutMS, &i.RateLimitPerMin, &i.MaxConcurrent, &i.MaxResponseBytes,
